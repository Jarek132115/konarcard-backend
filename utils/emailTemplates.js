...
const BRAND = {
  ...
  // Your production site (used to build absolute asset URLs)
  siteUrl: process.env.CLIENT_URL || "https://www.konarcard.com",
  supportEmail: "supportteam@konarcard.com",
  address: "KonarCard • London, UK",

  // Assets you put in frontend/public
  assets: {
    logo: "Logo-QR.png",
    footerBanner: "Banner.png", // CONNECT FASTER... image
    // (optional) hero you can use per email:
    hero: "Banner.png",
  },
};

// Build absolute URLs to public assets
const asset = (path) =>
  `${BRAND.siteUrl.replace(/\/+$/, "")}/${String(path).replace(/^\/+/, "")}`;
...
// The base email layout (adds bottom banner to EVERY email)
function renderEmail({
  ...
  ctaHtml = "",
  footerNote = "",
  // set heroUrl to show a top banner on a specific email, else omit
  heroUrl = null,
}) {
  const preheaderHtml = `
  ...
  const logoUrl = asset(BRAND.assets.logo);
  const footerBannerUrl = asset(BRAND.assets.footerBanner);
...
<body style="margin:0; padding:0; background:${BRAND.bodyBg}; font-family:Arial, Helvetica, sans-serif; color:${BRAND.text};">
  ${preheaderHtml}
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" bgcolor="${BRAND.bodyBg}">
    <tr>
      <td align="center" style="padding:24px 12px;">
        <table role="presentation" width="600" cellpadding="0" cellspacing="0" border="0"
               style="max-width:600px; width:100%; background:${BRAND.cardBg}; border-radius:12px; overflow:hidden; box-shadow:0 6px 24px rgba(0,0,0,.08);">

          <!-- Header -->
          <tr>
            <td align="center" bgcolor="${BRAND.dark}" style="padding:26px;">
              <img src="${logoUrl}" alt="${BRAND.name}" width="120" style="display:block; margin:0 auto 10px; max-width:140px;" />
          ...
          <!-- Bottom banner (always) -->
          <tr>
            <td align="center" style="padding:0;">
              <img src="${footerBannerUrl}"
                   alt="CONNECT FASTER. GROW SMARTER. YOUR DIGITAL BUSINESS CARD, REIMAGINED."
                   width="600" style="display:block; width:100%; max-width:600px; margin:0; border:0;" />
            </td>
          </tr>
...
function verificationEmailTemplate(name, code) {
  return renderEmail({
    ...
    preheader: "Your KonarCard verification code is inside.",
    headline: "Verify your email",
    bodyHtml: `
      <p style="margin:0 0 14px;">Hi ${name || "there"},</p>
      <p style="margin:0 0 16px;">Thanks for signing up to <strong>${BRAND.name}</strong>! Enter this code to verify your email:</p>
    ...
    `,
    ctaHtml: button("Open KonarCard", `${BRAND.siteUrl}/login`),
  });
}
...
function orderConfirmationTemplate(customerEmail, amountPaid) {
  return renderEmail({
    ...
    preheader: "Your KonarCard order has been confirmed.",
    ...
    `,
    ctaHtml: button("View your account", `${BRAND.siteUrl}/myprofile`),
  });
}
...
function subscriptionConfirmationTemplate(name, amountPaid, eventType) {
  ...
  return renderEmail({
    title: headline,
    preheader: "Konar Premium subscription update.",
    headline,
    bodyHtml,
    ctaHtml: button("Go to My Profile", `${BRAND.siteUrl}/myprofile`),
  });
}

// -------- Trial reminders (both now use the shared layout + footer banner) --

function trialFirstReminderTemplate(name) {
  return renderEmail({
    title: "Trial ending soon",
    preheader: "Don’t lose access to your card.",
    ...
    `,
    ctaHtml: button("Upgrade to Premium", `${BRAND.siteUrl}/subscription`),
  });
}
...
function trialFinalWarningTemplate(name) {
  return renderEmail({
    ...
    preheader: "Final reminder to keep your card live.",
    headline: "Last chance – your trial ends today",
    bodyHtml: `
      <p style="margin:0 0 14px;">Hi ${name || "there"},</p>
    ...
    `,
    ctaHtml: button("Subscribe and keep my card", `${BRAND.siteUrl}/subscription`, "#dc3545"),
  });
}
